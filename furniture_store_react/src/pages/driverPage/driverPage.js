...
const DriverPage = () => {
    ...
    useEffect(() => {
        axiosInstance.get("pendingOrders")
            .then((res) => {
                setOrders(res.data)
            })
    }, [])
                ...
                <main className="content driver-main body">
                    <section className="col">
                        ...
                        <div className="d-flex justify-content-center">
                            {orders.length == 0 ? <div className="no-text">There is no orders to shipped.</div> :
                                        ...
                                        {orders.map((order) => {
                                            ...
                                            return (
                                                <tr key={order.id}>
                                                    ...
                                                    <td className=""><Link to={`/driver/${order.id}`} className="ship-link"><button className="ship-btn">Ship Order</button></Link></td>
                                                </tr>
                                            )
                                        })}
                                    </tbody>

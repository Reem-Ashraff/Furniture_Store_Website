...
const CategoryEdit = () => {
    ...
    useEffect(() => {
        ...
        axiosInstance.get(`getCategory/${params.id}`)
            .then((res) => {
                                ...
                                <form action="post" className="form" onSubmit={handleSubmit}>
                                    ...
                                    <div className="d-flex flex-column inputs">
                                        ...
                                        <select className="form-select input w-100" name="parent_id" onChange={(e) => handleChange(e)}>
                                            <option value="" selected={formData.parent_id == ""}>No Parent Category</option>
                                            {categories.map((cat) => {
                                                return (
                                                    <option key={cat.id} value={cat.id} selected={cat.id === formData.parent_id}>{cat.category_name}</option>
                                                )
                                            })}
                                        </select>
